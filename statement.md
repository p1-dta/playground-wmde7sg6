...
Changez le lexique en remplaçant par exemple les parenthèses par des crochets, syntaxiquement valide,
des "mots" connus de Python, et il *peut* ne pas y avoir de plantage :

```python runnable
print['Hello World!']
```

Sauf... Que ça plante. Pourquoi ? Pour la simple et bonne raison que ce code n'a plus le même sens qu'avant.

> Bien sûr, on change les mots, on change le sens : 
"Un grand arbre" ne signifie pas la même chose que "Un petit arbre"

Un nom suivi de parenthèses est un appel (`Call`), mais un nom suivi de crochets est un subscript (`Subscript`).

```python runnable
import ast

print(ast.parse('print("hello world")').body.pop().value.__class__.__name__)
print(ast.parse('print["hello world"]').body.pop().value.__class__.__name__)
```
...
"ce qu'il veut dire", est différente. En changeant le lexique utilisé, nous avons changé le sens du code. 
La ligne ajoutée au dessus ne sert qu'à rendre ce code "logique", mais si vous retirez la ligne, 
le lexique est le même, mais il n'a plus un sens logique.
Python va donc planter, affichant une erreur car vos instructions n'ont pas de sens.
